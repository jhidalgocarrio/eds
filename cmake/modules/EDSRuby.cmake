...
#
# eds_ruby_rice_extension(your_extension_ruby ${SOURCES})
...
option(EDS_RUBY_TESTS_EXPLICIT_AUTORUN "Explicitly add -rminitest/autorun to run ruby tests (DEPRECATED)" ON)

# eds_add_ruby_package(NAME
...
function(eds_add_ruby_package NAME)
    set(mode START)
    set(required OFF)
    set(quiet OFF)
    foreach(arg ${ARGN})
        ...
        elseif (mode STREQUAL "EXT_NAME")
            if (NOT IS_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/ext/${arg}")
                message(FATAL_ERROR "eds_add_ruby_package: ${arg} was expected to be a subdirectory of ext/, but is not")
            endif()
        ...
        elseif (mode STREQUAL "EXT_FILES")
            ...
            message(FATAL_ERROR "eds_add_ruby_package: unexpected argument ${arg} in mode '${mode}'")
        endif()
    endforeach()
    ...
    if (${NAME}_AVAILABLE)
        foreach(extname ${extension_names})
            if (${extname}_TYPE STREQUAL "EXT_PLAIN")
                eds_ruby_extension(${extname} ${${extname}_FILES})
            elseif (${extname}_TYPE STREQUAL "EXT_RICE")
                eds_ruby_rice_extension(${extname} ${${extname}_FILES})
        ...
        endforeach()
        eds_ruby_library(${NAME} ${pure_ruby_files})

        if (EDS_TEST_ENABLED AND IS_DIRECTORY test)
            enable_testing()
            eds_ruby_test(${NAME})
        endif()
        ...
        if (YARD_EXECUTABLE)
            eds_ruby_doc(${NAME})
            eds_add_dummy_target_dependency(doc doc-${NAME}-ruby)
        endif()
    elseif (required)
...
if (NOT RUBY_FOUND)
    ...
    function(EDS_RUBY_LIBRARY libname)
    ...
    function(EDS_LOG_MIGRATION)
        if (EXISTS ${CMAKE_SOURCE_DIR}/src/log_migration.rb)
            ...
            install(FILES ${CMAKE_BINARY_DIR}/log_migration-${PROJECT_NAME}.rb
                    DESTINATION share/eds/log/migration)
        endif()
    endfunction()

    function(EDS_TYPELIB_RUBY_PLUGIN)
        install(FILES ${ARGN}
            DESTINATION share/typelib/ruby)
    endfunction()

    function(EDS_LOG_EXPORT)
        if (EXISTS ${CMAKE_SOURCE_DIR}/src/log_export.rb)
            ...
            install(FILES ${CMAKE_BINARY_DIR}/log_export-${PROJECT_NAME}.rb
                    DESTINATION share/eds/log/export)
        endif()
    endfunction()
    ...
    #
    # If the EDS_TEST_LOG_DIR CMake variable is set, a test report will be
    ...
    function(EDS_RUBY_TEST TARGET)
        ...
        foreach(arg ${ARGN})
            ...
            elseif (mode STREQUAL "FILES")
                ...
                message(FATAL_ERROR "trailing arguments ${arg} to eds_ruby_test")
            endif()
        endforeach()
        ...
        if (NOT has_test_args)
            if (IS_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/test)
                ...
                message(FATAL_ERROR "eds_ruby_test: called without test files, and there is no test/ folder")
            endif()
        endif()
        ...
        if (EDS_TEST_LOG_DIR)
            ...
            list(APPEND __minitest_args
                --junit
                --junit-filename=${EDS_TEST_LOG_DIR}/${TARGET}.junit.xml
                --junit-jenkins
            )
        endif()

        if (EDS_RUBY_TESTS_EXPLICIT_AUTORUN)
...
ELSEIF(NOT RUBY_EXTENSIONS_AVAILABLE)
    ...
    function(EDS_RUBY_EXTENSION target)
	...
	SET_SOURCE_FILES_PROPERTIES(${ARGN} PROPERTIES COMPILE_FLAGS "${RUBY_CFLAGS}")
        eds_library_common(${target} MODULE ${ARGN})
        IF(APPLE)
    ...
    macro(EDS_RUBY_RICE_EXTENSION target)
        ...
        EDS_RUBY_EXTENSION(${target} ${ARGN})
